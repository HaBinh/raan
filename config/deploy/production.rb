...
namespace :custom do
  task :setup_container do
    on roles(:docker) do |host|
      ...
      execute "source ~/.bash_profile && cd #{deploy_to}/current && bundle exec rake db:migrate && bundle exec pumactl -P tmp/pids/puma.pid stop && bundle exec pumactl -P tmp/pids/puma.pid start"
    end
  end
end
